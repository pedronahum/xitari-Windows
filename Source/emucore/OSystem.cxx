...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void OSystem::setBaseDir(const std::string& basedir)
{
  myBaseDir = basedir;
...
void OSystem::toggleFrameBuffer()
{
  ...
  std::string video = mySettings->getString("video");
  if(video == "soft")
    video = "gl";
  else if(video == "gl")
    video = "soft";
...
bool OSystem::createConsole(const std::string& romfile)
{
  ...
  if(romfile == "")
  {
    ...
    if(myRomFile == "")
    {
      std::cerr << "ERROR: Rom file not specified ..." << std::endl;
      return false;
    }
  }
  else
  ...
  if(openROM(myRomFile, md5, &image, &size))
  {
    ...
    if(queryConsoleInfo(image, size, md5, &cart, props))
    {
      ...
      if(mySettings->getBool("showinfo"))
        std::cerr << "Game console created:" << std::endl
    ...
    {
      std::cerr << "ERROR: Couldn't create console for " << myRomFile << " ..." << std::endl;
      retval = false;
    }
  }
  else
  {
    std::cerr << "ERROR: Couldn't open " << myRomFile << " ..." << std::endl;
    retval = false;
  }
...
void OSystem::deleteConsole()
{
  if(myConsole)
  {
    ...
    //   std::cerr << "Game console stats:" << std::endl
    //        << "  Total frames drawn: " << myTimingInfo.totalFrames << std::endl
    //        << "  Total time (sec):   " << executionTime << std::endl
    //        << "  Frames per second:  " << framesPerSecond << std::endl
    //        << std::endl;
    // }
    delete myConsole;  
    myConsole = NULL;
  ...
  if(openROM(romfile, md5, &image, &size))
  {
    ...
    if(queryConsoleInfo(image, size, md5, &cart, props))
    {
      ...
      if(console)
        ...
        buf << "ERROR: Couldn't get ROM info for " << romfile << " ..." << std::endl;
                               ...
                               Cartridge** cart, Properties& props)
{
  ...
  myPropSet->getMD5(md5, props);
    ...
    if (s == "A") {
        std::cerr << "Setting Left Player's Difficulty to mode: A" << std::endl;
    }
