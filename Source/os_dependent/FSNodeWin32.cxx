...
class WindowsFilesystemNode : public AbstractFilesystemNode
{
  public:
    ...
    WindowsFilesystemNode(const std::string &path);
    WindowsFilesystemNode(const WindowsFilesystemNode* node);

    virtual std::string displayName() const { return _displayName; }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
static const char* lastPathComponent(const std::string& str)
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
static std::string validatePath(const std::string& p)
{
  std::string path = p;
  if(p.size() < 2 || p[1] != ':')
    path = "c:";
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
AbstractFilesystemNode* FilesystemNode::getNodeForPath(const std::string& path)
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
WindowsFilesystemNode::WindowsFilesystemNode(const std::string& path)
{
...
AbstractFilesystemNode* WindowsFilesystemNode::parent() const
{
  ...
  if (_path.size() > 3)
  {
    ...
    p->_path = std::string(start, end - start);
    p->_isValid = true;
    p->_isDirectory = true;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool AbstractFilesystemNode::fileExists(const std::string& path)
{
  WIN32_FILE_ATTRIBUTE_DATA attr;
  auto widestr = std::wstring(path.begin(), path.end());
  auto widecstr = widestr.c_str();
  BOOL b = GetFileAttributesEx(widecstr, GetFileExInfoStandard, &attr);

  return ((b != 0) && !(attr.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool AbstractFilesystemNode::dirExists(const std::string& path)
{
  WIN32_FILE_ATTRIBUTE_DATA attr;
  auto widestr = std::wstring(path.begin(), path.end());
  auto widecstr = widestr.c_str();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool AbstractFilesystemNode::makeDir(const std::string& path)
{
  auto widestr = std::wstring(path.begin(), path.end());
  auto widecstr = widestr.c_str();
  return CreateDirectory(widecstr, NULL) != 0;
}
